...
Infra wires saga mechanics (from `forgeerp-events`) into running processes:

- `saga/` module:
  - `SagaRepository`: persist saga state/events into the event store (append-only)
  - `CommandExecutor` trait: run typed commands generated by a saga
  - Reference saga: Sales → Invoice → Ledger (Accounts Receivable)
    - Transitions:
      - `sales.order.confirmed` → Issue Invoice (lines sourced from SalesOrder read model)
      - `invoicing.invoice.issued` → Post journal (debit AR, credit Revenue)
    - Compensation:
